    ...
    >
      ...
      <div class="subscriptions-menu-section">
        ...
        <div v-if="subscriptionsExpanded" class="subscriptions-menu-body">
          <div v-if="subscriptionError" class="subscriptions-menu-error">{{ subscriptionError }}</div>
          ...
          >
            ...
            >
              ...
              <a :href="subscriptionLogin.session.verification_url" target="_blank" rel="noopener noreferrer">
...
async function pollOpenAILogin() {
  ...
  subscriptionError.value = null;
  try {
    const result = await api.pollOpenAISubscriptionLogin(subscriptionLogin.value.session.session_id);
    if (result.done) {
